...
from junior.forms import EditCompanyForm, EditVacancyForm, EditResumeForm
from junior.models import Vacancy, Company, Application, Resume
...
class MyCompanyView(View):
    def get(self, request):
        if not Company.objects.filter(owner_id=request.user.id).exists():
            return render(request, 'junior/company-create.html')
        company = Company.objects.get(owner_id=request.user.id)
        context = {
            'form': EditCompanyForm(instance=company),
        }
        return render(request, 'junior/company-edit.html', context)
    ...
    def post(self, request):
        ...
        return render(request, 'junior/company-edit.html', {'form': form})
...
class MyCompanyVacanciesView(View):
    def get(self, request):
        ...
        return render(request, 'junior/vacancy-list.html', context)


@method_decorator(login_required, name='dispatch')
class MyCompanyVacancyView(View):
    def get(self, request, vacancy_id):
        vacancy = get_object_or_404(Vacancy, id=vacancy_id, company__owner__id=request.user.id)
        applications = Application.objects.filter(vacancy_id=vacancy_id)
        context = {
            'form': EditVacancyForm(instance=vacancy),
            'applications': applications,
        }
        return render(request, 'junior/vacancy-edit.html', context)
    ...
    def post(self, request, vacancy_id):
        ...
        if form.is_valid():
            ...
            return redirect('vacancy_edit', vacancy_id=vacancy_id)
...
class MyCompanyVacancyCreate(View):
    ...
    def post(self, request):
        ...
        return render(request, 'junior/vacancy-edit.html', {'form': form})
...
class ResumeView(View):
    def get(self, request):
        if not Resume.objects.filter(user__id=request.user.id).exists():
            return render(request, 'junior/resume-create.html')
        resume = Resume.objects.get(user__id=request.user.id)
        context = {
            'form': EditResumeForm(instance=resume),
        }
        return render(request, 'junior/resume-edit.html', context)
    ...
    def post(self, request):
        ...
        return render(request, 'junior/resume-edit.html', {'form': form})
